...
class TaskMetric:
    def __init__(self, train_tasks, pri_tasks, batch_size, epochs, dataset, weather, include_mtl=False):
    ...
    def compute_metric(self, only_pri=False):
        ...
        if self.include_mtl:
            ...
            if self.dataset == 'nyuv2':
                stl = {'seg': 0.4656, 'depth': 0.4057, 'normal': 23.41}
            elif self.dataset == 'cityscapes':
                if self.weather == 'clear':
                    # stl = {'seg': 0.5620, 'part_seg': 0.5274, 'disp': 0.84}
                    stl = {'seg': 0.5420, 'part_seg': 0.5182, 'disp': 0.8151}
                elif self.weather == 'foggy':
                    stl = {'seg': 0.4781, 'part_seg': 0.5230, 'disp': 0.9871}
                elif self.weather == 'rainy':
                    stl = {'seg': 0.4685, 'part_seg': 0.5132, 'disp': 1.1601}
...
def keypoint_distance(points1, points2):
    ...
    return torch.sum((points1 - points2) ** 2, dim=1)
...
def k_center_loss(fake_images, real_images):
    ...
    fake_keypoints = torch.mean(fake_images, dim=[1,2])
    real_keypoints = torch.mean(real_images, dim=[1,2])
    ...
    distances = torch.norm(fake_keypoints - real_keypoints, dim=-1)
...
def image_save_segmentation(test_data, original_image, save_path, alpha=0.8):
    ...
    Args:
        test_data (torch.Tensor): The input tensor of shape (C, H, W) representing C class segmentation results,
                                  or (H, W) representing predicted classes in range [-1, 18].
    ...
    # Check if test_data is already labels or needs argmax
    if len(test_data.shape) == 3:
        predicted_classes = np.argmax(test_data, axis=0)
    else:
        predicted_classes = test_data

    # Adjust the class range from [-1, 18] to [0, 19]
    # if np.min(predicted_classes) == -1:
    # predicted_classes = predicted_classes + 1

    # Create a colormap
    num_classes = 20  # [-1, 18] -> [0, 19]
